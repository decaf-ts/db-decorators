...
// @ts-ignore
import {DBRepo, GenericCaller, OtherTransactionalRepository, TransactionalRepository} from "./TestRepository";
import {getLogger, LOGGER_LEVELS} from "@decaf-ts/logging";
import type {Callback, Err} from "@decaf-ts/logging";
import {Transaction} from "../../src";
import {SyncronousLock} from "../../src";

jest.setTimeout(30000)
if (process.env["GITLAB_CI"])
  jest.setTimeout(3 * 30000)
else
...
describe(`Transactional Context Test`, function () {
  ...
  it(`Schedules transactions properly`, (testFinished) => {
    ...
    const onEndPromise = async (err?: Err) => {
    ...
    consumerRunner.run(count, 100, times, true, (err: Err) => {
      if (err)
  ...
  describe("Handles different transactional methods within the same transaction", () => {
    ...
    it("Handles calls to multiple transactional methods within the same Async transactional function", (callback) => {
      ...
      caller.runAsync(tm, (err: Err, model1?: TestModelAsync, model2?: TestModelAsync) => {
  ...
  describe("properly Logs transaction method calls", () => {
    ...
    it("Logs a simple transaction properly", (callback) => {
      ...
      repo.create("key1", model, (err: Err, created?: TestModelAsync) => {
        if (err || !created)
          return callback(err)
