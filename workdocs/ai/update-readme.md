...
task 5 - if there's a file `<base_path>/workdocs/ai/focus-areas.md`, read it. if not ask the user for focus points for the description and examples (user can say no). if the user provides them, crete the file for future reference.
...
NOTE: don't forget to write your updates to the files. after every task, write the changes to the files.